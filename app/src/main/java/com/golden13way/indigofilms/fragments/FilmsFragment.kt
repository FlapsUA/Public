package com.golden13way.indigofilms.fragments

import android.os.Bundle
import androidx.fragment.app.Fragment
import android.view.LayoutInflater
import android.view.View
import android.view.ViewGroup
import androidx.lifecycle.*
import com.golden13way.indigofilms.R
import com.golden13way.indigofilms.adapters.AdapterFilms
import com.golden13way.indigofilms.adapters.RecyclerViewAdapter
import com.golden13way.indigofilms.databinding.FragmentFilmsBinding

import com.golden13way.indigofilms.viewModels.MainActivityViewModel


lateinit var BASE_URL : String

// TODO: Rename parameter arguments, choose names that match
// the fragment initialization parameters, e.g. ARG_ITEM_NUMBER
//private const val page = "param1"
//private const val type = "param2"

/**
 * A simple [Fragment] subclass.
 * Use the [FilmsFragment.newInstance] factory method to
 * create an instance of this fragment.
 */
class FilmsFragment : Fragment() {
    // TODO: Rename and change types of parameters
    /*private var param1: String? = null
    private var param2: String? = null*/

    private lateinit var recyclerViewAdapter: RecyclerViewAdapter
    private lateinit var mainActivityViewModel: MainActivityViewModel
    lateinit var bind: FragmentFilmsBinding
    private val adapter = AdapterFilms()

    var page = "1"
    var type = "film"

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        arguments?.let {

            /* param1 = it.getString(ARG_PARAM1)
            param2 = it.getString(ARG_PARAM2)*/
        }


    }

    override fun onCreateView(
        inflater: LayoutInflater,
        container: ViewGroup?,
        savedInstanceState: Bundle?
    ): View? {
        val view = inflater!!.inflate(R.layout.fragment_films, container, false)
        return view
    }



    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        super.onViewCreated(view, savedInstanceState)





    }





    /*private fun getFilmsList(){
        val IFVM : IFViewModel
        val application : Application = Application()
        var apiRequest : ApiRequest

        IFVM = IFViewModel(application)
        val fl = ViewModelProvider.
       val filmslist = IFVM.updateFilmList()









        *//*val api = Retrofit.Builder()
            .baseUrl(BASE_URL)
            .addConverterFactory(GsonConverterFactory.create())
            .build()
            .create(ApiRequest::class.java)

        GlobalScope.launch(Dispatchers.IO) {
            val response = api.getFlimsList().awaitResponse()
            if (response.isSuccessful){
                val data = response.body()!!
                Log.d("LogD", data.data.items[0].title)
                withContext(Dispatchers.Main){

                    rc(data)
                }
            }
        }*//*
    }*/


    companion object {
        /**
         * Use this factory method to create a new instance of
         * this fragment using the provided parameters.
         *
         * @param param1 Parameter 1.
         * @param param2 Parameter 2.
         * @return A new instance of fragment FilmsFragment.
         */
        // TODO: Rename and change types and number of parameters
        @JvmStatic
        fun newInstance() =
            FilmsFragment().apply {
                arguments = Bundle().apply {

                }
            }
    }
}
/*
private fun <T> MutableLiveData<T>.observe(filmsFragment: FilmsFragment, observer: Observer<Films>) {

}*/
